...
public class PlayerControllerbak : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        ApplyGravity();
        MoveCharacter();
        if (hasAnimator) UpdateAnimation();
        //UpdateAnimation();
    }
    ...
    private void InitializeComponents()
    {
        ...
        if (!TryGetComponent(out audioSource)) // AudioSource 추가
        {
    ...
    private void HandleInput()
    {
        ...
        {
            yVelocity = jumpSpeed;
            if (hasAnimator && !animator.GetBool(animIDJump)) // 점프가 아직 활성화되지 않았을 때만
            {
                animator.SetBool(animIDJump, true);
            }
        }
        else if (hasAnimator && animator.GetBool(animIDJump)) // 점프가 끝났을 때만
        {
